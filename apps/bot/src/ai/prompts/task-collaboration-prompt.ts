...
add_task_comment: { taskQuery?, taskTitle?, comment?, taskId?, mentionUserHints? } — раздели поиск задачи и текст комментария; после «что»/«:» → comment. Если фраза содержит «для <имя>», «<имя> в комментарии», «упомяни <имя>» — добавь mentionUserHints: ["<имя>"], но НЕ переназначай задачу.
list_task_comments: { taskQuery?, taskTitle?, taskId? } — только чтение комментариев; requiresConfirmation: false.
mention_in_task: { userHint, taskTitle, text? } — только когда основная цель позвать человека в задачу, без конкретного текста комментария.
