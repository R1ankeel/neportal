...
- Часть read/write API требует **`actorUserId`** (видимость проектов и задач по членству; OWNER видит всё). Web передаёт через query, бот - id привязанного пользователя.
...
| **Note** | Личная текстовая заметка (`creatorId`), без проекта; источник `WEB` / `TELEGRAM_*`; Web `/notes` |
...
4. Web: `/notes?actorUserId=…` (личные заметки) и/или задачи в проекте, который выбрал бот (один доступный проект - auto-select, несколько - выбор или `projectHint`).
