...
### Checkpoint: semantic input vs кнопки

Короткий ориентир для voice/semantic слоя: где по-прежнему нужен semantic input, а где сценарий уже button-first.

**Остаётся semantic input (распознаём смысл свободного текста/голоса):**
...
- меню редактирования confirmation («что редактируем?» + «сохранить без изменений» + «отменить»).

**Практическое правило:** если у пользователя активен `pending` с inline-клавиатурой (`confirmation`/`choice`), приоритет у нажатия кнопки; текстовые ответы (`да`, `нет`, `1`, `2`, `отмена`) считаются fallback и не должны быть отдельной целью для voice-only распознавания.
...
- Stage 5.2: Telegram `message:voice` подключён в тестовом режиме intake-only: бот распознаёт короткое voice через SpeechKit и возвращает текст пользователю.
- На этом этапе intent execution из voice не выполняется: распознанный текст не отправляется в `parseTextIntent`/business flow.
- Если активен `pending` с кнопками/ожиданием ответа, voice не обходит state-machine: нужно завершить текущее действие кнопками или текстом.
